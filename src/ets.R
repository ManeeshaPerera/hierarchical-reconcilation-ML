...
f_model <- function (x, frequency, horizon){
  model_fitting <- ets(ts(as.numeric(x), frequency = frequency))
...
model_results_write <- function(df, data, frequency, horizon){
  ...
  write.table(residual_df, paste(data, "ets_fitted.csv", sep = '_'),
              col.names = TRUE, row.names = FALSE, sep = ",")
  write.table(forecasts_df, paste(data, "ets_forecasts.csv", sep = '_'),
              col.names = TRUE, row.names = FALSE, sep = ",")
}


run_ets<- function (filename, freq, horizon, name) {
