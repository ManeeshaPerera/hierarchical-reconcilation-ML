...
class MLReconcile:
    def __init__(self, seed_value, actual_data, fitted_data, forecasts, fitted_transform_matrix, fc_transform_matrix,
        ...
        self.transform_matrix_fitted = fitted_transform_matrix
        self.transform_matrix_fc = fc_transform_matrix
        self.number_of_levels = number_of_levels
        self.actual_transpose = None
        self.fitted_transpose = None  # this will also include the transform matrices
        self.forecast_transpose = None  # this will also include the transform matrices
    ...
    def _run_initialization(self, seed_value, tune_hyper_params, hyper_params_tune, best_hyper_params):
        ...
        transpose_fitted = self._transpose_data(self.transform_matrix_fitted)
        self.fitted_transpose = transpose_fitted # input is set to transformed matrix only

        transpose_fc = self._transpose_data(self.transform_matrix_fc)
        self.forecast_transpose = transpose_fc # input forecasts are set to transformed values

        fitted_vals = self._transpose_data(self.fitted_data)
        fitted_vals.columns = [f'fitted#{val}' for val in fitted_vals.columns]
        self.actual_transpose = pd.concat([self.actual_transpose, fitted_vals], axis=1)
    ...
    def _custom_loss(self, reconciliation_loss_lambda):
        ...
        def loss(data, y_pred):
            ...
            fitted_start_idx = start_index_bottom_ts + bottom_level_ts
            fitted_bottom_start_idx = fitted_start_idx + start_index_bottom_ts  # this will give the fitted bottom level values

            actual = data[:, start_index_bottom_ts: fitted_start_idx]

            prediction_vals = data[:, fitted_bottom_start_idx:] + all_predictions

            # calculate the bottom time series loss
            loss_fn = tf.losses.MeanSquaredError()
            prediction_error = loss_fn(actual, prediction_vals)
            ...
            for key in hierarchy:
                ...
                fitted_bottom_start = fitted_bottom_start_idx + lower_index
                fitted_bottom_end = fitted_bottom_start_idx + (high_index + 1)

                lower_node_ts_pred = data[:, fitted_bottom_start:fitted_bottom_end] + lower_node_ts_pred
    ...
    def validate_model(self, hyperparams, data_dic):
        def val_loss_fn(y_actual, y_pred):
            number_of_bottom_ts, bottom_level_index = self.hierarchy.get_bottom_level_ts_info()
            fitted_start_idx = bottom_level_index + number_of_bottom_ts
            fitted_bottom_start_idx = fitted_start_idx + bottom_level_index
            ...
            if self.validate_hf_loss:  # if the validation loss is calculated to the whole hierarchy
                ...
                for higher_node_index in hf_ts_indexes:
                    ...
                    lower_node_ts_pred = y_actual[:, fitted_bottom_start:fitted_bottom_end] + y_pred[:,
                                                                                              lower_index: (
                                                                                                      high_index + 1)]  # get predictions for all bottom level nodes
                ...
                validation_loss = np.mean(
                    np.sqrt(tf.losses.MSE(y_actual.values[:, 0:fitted_start_idx], bottom_up_predictions)))
            else:
                validation_loss = np.mean(
                    np.sqrt(tf.losses.MSE(y_actual.values[:, bottom_level_index:fitted_start_idx], y_pred)))
