...
for iter_window in range(1, windows + 1):
    if iter_window % n == 1:
        ...
        result_r = hts_benchmarks(dataset_name, '', '', 'arima', iter_window, iter_window, 'rolling_window')
        last_iter = iter_window
    else:
        result_r = hts_benchmarks(dataset_name, '', '', 'arima', last_iter, iter_window, 'rolling_window')
