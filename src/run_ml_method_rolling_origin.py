...
def run_ml_reconciliation(dataset, rolling_iter, ml_method, lambda_range_run, seed_to_run, seed_array, hf_levels,
    ...
    if not run_saved_model:
        ...
        # model was trained with these values
        df_actual = pd.read_csv(f"rolling_window_experiments/{dataset}/actual_{prev_window}.csv")
        df_fitted = pd.read_csv(f"rolling_window_experiments/{dataset}/{model}_fitted_{prev_window}.csv")
    ...
    if dir_store:
        path_dir = f'{dataset}/{dir_store}'
    else:
        path_dir = dataset

    forecast_path = f'rolling_window_experiments/hts/{path_dir}'
    hyper_param_path = f'rolling_window_experiments/hyper_params/{path_dir}'
...
if __name__ == '__main__':
    ...
    if rolling_window:  # training at each iteration
        rolling_window = int(rolling_window)
        dir_store = f'{data}_new'
        ...
        dir_store = False
        times = []  # training and prediction times
        ...
        for rolling_window in range(1, num_rolling_windows + 1):
            ...
            if rolling_window % n == 1:
                ...
                run_ml_reconciliation(data, rolling_window, ml_method_name, lambda_case, seed_value, seed_runs,
                                      number_of_levels,
                                      tune_lambda, run_saved_model=True, saved_models=saved_models)
                et = time.time()
                times.append(et - st)
