...
class MLReconcile:
    ...
    def _run_initialization(self, seed_value, tune_hyper_params, hyper_params_tune, best_hyper_params):
        ...
        transpose_fitted.columns = [f'residual#{val}' for val in transpose_fitted.columns]
        self.fitted_transpose = pd.concat([self.fitted_transpose, transpose_fitted], axis=1)

        transpose_fc = self._transpose_data(self.transform_matrix_fc)
        transpose_fc.columns = [f'residual#{val}' for val in transpose_fc.columns]
        self.forecast_transpose = pd.concat([self.forecast_transpose, transpose_fc], axis=1)
    ...
    def build_and_compile_model(self, hyperparams):
        ...
        if self.remove_skip:
            ...
            outputs = tf.add(model_outputs, bottom_level_fitted)  # skip connection from input to bypass the model
    ...
    def _train_model_with_seeds(self, data_dic):
        ...
        for run in range(len(self.seed_runs)):
            ...
            if not self.run_saved_models:
                ...
                saved_model_path = f'{self.model_path}/SEED_{run}'
                if not os.path.exists(saved_model_path):
                    os.makedirs(saved_model_path)
                save_options = tf.saved_model.SaveOptions(experimental_io_device='/job:localhost')
                ml_rec_model.save(saved_model_path, options=save_options)
            else:
                # retrieve the model we saved in a previous rolling window iteration
                # ml_rec_model = self.saved_models[run] # load model

                ml_rec_model = tf.saved_model.load(f'{self.model_path}/SEED_{run}',
