        ...
        for error in ['MASE', 'SMAPE']:
            if error == 'MASE':
                ...
                adjusted_error = mean_absolute_scaled_error(y_true=actual_test_ts, y_pred=ts_fc_adjusted,
                                                            y_train=actual_train_ts)
                errors.append([ts_names[ts], 'original', 'MASE', original_error, levels[ts]])
                errors.append([ts_names[ts], 'adjusted', 'MASE', adjusted_error, levels[ts]])
                ...
                errors.append([ts_names[ts], 'original', 'SMAPE', original_error, levels[ts]])
                errors.append([ts_names[ts], 'adjusted', 'SMAPE', adjusted_error, levels[ts]])

    errors = pd.DataFrame(errors)
    errors.columns = ['ts_name', 'forecast_type', 'error_metric', 'error', 'level']
